"""
AUX DINOv2 visual matcher — experimental variant for thesis evaluation.

Goes in: backend/app/services/visual_matcher_dino_aux.py

Same modifications as visual_matcher_aux.py (uniform filter + extended crops),
but using DINOv2 (facebook/dinov2-base) embeddings via Hugging Face Transformers.

Loads from /app/data/brand_embeddings_dino_aux.pkl.
"""
import pickle
import threading
...
EMBEDDINGS_PATH_DINO = Path("/app/data/brand_embeddings_dino_aux.pkl")
...
def _load() -> bool:
    ...
    with _lock:
        ...
        try:
            ...
            print(f"[visual_matcher_dino_aux] load error: {e}")
            return False


def is_dino_aux_available() -> bool:
...
def _compute_query_embeddings(image_path: str) -> list:
    try:
        ...
        print(f"[visual_matcher_dino_aux] image load error: {e}")
        return []
...
def match_brand_dino_aux(screenshot_path: str, threshold: float = 0.85) -> dict:
