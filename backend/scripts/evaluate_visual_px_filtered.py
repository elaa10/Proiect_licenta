...
from app.services.visual_matcher_px_filtered import match_brand_px_filtered, _load
...
def evaluate():
    print("Loading PX+Filter matcher...")
    if not _load():
        print("ERROR: brand_embeddings_px_filtered.pkl not found. "
              "Run scripts/init_brand_db_px_filtered.py first.")
        sys.exit(1)
    print("PX+Filter matcher ready.\n")
    ...
    for b_idx, brand_dir in enumerate(brand_dirs):
        ...
        for sample_dir in sample_dirs:
            ...
            try:
                result = match_brand_px_filtered(str(shot))
            except Exception:
    ...
    results = {
        "model": "CLIP ViT-B/32 — PX+Filter (pixel multi-crop + uniform-color filter)",
        "strategy": "4 full-width pixel crops (top_150, top_300, top_500, mid_300) "
                    "with std-based uniform skip (threshold 12), applied at both "
    ...
    out = RESULTS_DIR / "visual_evaluation_px_filtered.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print(f"\n{'='*60}")
    print(f"PX+FILTER RESULTS ({total_samples} samples)")
    print(f"{'='*60}")
    print(f"  Precision : {p:.2%}")
    print(f"  Recall    : {r:.2%}")
    print(f"  F1        : {f1:.2%}")
    print(f"\nResults written to: {out}")
    print(f"Reference: backend/results/visual_evaluation_px.json (PX, no filter)")
