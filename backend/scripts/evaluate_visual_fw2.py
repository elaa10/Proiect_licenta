...
from app.services.visual_matcher_fw2 import match_brand_fw2, _load
...
def evaluate():
    print("Loading FW2 CLIP matcher (full-width ≤50% + uniform filter + login refs)...")
    if not _load():
        print("ERROR: brand_embeddings_fw2.pkl not found. "
              "Run scripts/init_brand_db_fw2.py first.")
        sys.exit(1)
    print("FW2 matcher ready.\n")
    ...
    for b_idx, brand_dir in enumerate(brand_dirs):
        ...
        for sample_dir in sample_dirs:
            ...
            try:
                result = match_brand_fw2(str(shot))
            except Exception:
    ...
    results = {
        "model": "CLIP ViT-B/32 — FW2 (full-width ≤50% + uniform filter + login refs)",
        "strategy": (
            "4 full-width crops capped at y2<=0.50: "
            "top_narrow(0-20%), top_medium(0-35%), top_half(0-50%), mid_upper(10-45%) "
            "— with std-based uniform skip (threshold 12) "
            "— login references added for LinkedIn, Adobe, Dropbox, Spotify, eBay, Booking, Glovo"
        ),
    ...
    out = RESULTS_DIR / "visual_evaluation_fw2.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    print(f"\n{'='*60}")
    print(f"FW2 CLIP RESULTS ({total_samples} samples)")
    print(f"{'='*60}")
    ...
    print(f"  visual_evaluation.json           — single crop 300px            (F1=0.095)")
    print(f"  visual_evaluation_multicrop.json — proportional crops, no filter (F1=0.077)")
    print(f"  visual_evaluation_aux.json       — 9 mixed crops + filter        (F1=0.258)")
    print(f"  visual_evaluation_px.json        — pixel crops, no filter        (run separately)")
    print(f"  visual_evaluation_fw.json        — full-width prop + filter      (run separately)")
    print(f"  visual_evaluation_fw2.json       — full-width ≤50% + login refs  (this run)")
