"""
Build brand_embeddings_fw2.pkl — FW2 variant with login references added.

Goes in: backend/scripts/init_brand_db_fw2.py

Screenshot lookup order (nothing gets overwritten):
  1. /app/screenshots/brands/{name}_{label}.png   — existing screenshots, reused as-is
  2. /app/screenshots/brands2/{name}_{label}.png — new captures go here

New login references added vs. init_brand_db.py:
  - linkedin, adobe, dropbox, spotify, ebay, booking, glovo

These were previously excluded because their login pages contain OAuth buttons
(Google, Apple, Facebook). With the FW2 crop strategy (y2 <= 0.50) the OAuth
buttons, which appear in the lower half of the page, are never included in the
embeddings — making it safe to use these login pages as references.

IMPORTANT: before running, visually verify each new login screenshot in
brands2/ to confirm the brand logo is visible in the top half and any
OAuth buttons are fully below the 50% mark.

Output: /app/data/brand_embeddings_fw2.pkl
"""
import asyncio
import pickle
...
BRANDS_DIR = Path("/app/screenshots/brands2")  # new captures go here
...
async def main():
    ...
    if EMBEDDINGS_PATH.exists():
        ...
        print(f"Resuming: {len(embeddings)} brands already in fw2 pkl\n")
